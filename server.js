...
app.get(['*/:param', '*'], (req, res) => {
  ...
  const assets = JSON.parse(fs.readFileSync(path.join(`${process.cwd()}` + "/build/public/asset-manifest.json"), 'utf8'));
  ...
  getDataFromTree(App).then(() => {
    ...
    const html = <HTML content={content} state={initialState} helmet={helmet} assets={assets} />;
